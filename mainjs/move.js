...
$(document).ready(function () {
  ...
  $('.btn1').css('background', ' #da291c'); //첫번째 불켜
  $('.btn1').css('width', '100'); // 버튼의 너비 증가
  $('.ps').html('<i class="fa-solid fa-pause"></i>')
  $('.gallery .link1').fadeIn('slow'); //첫번째 이미지 보인다..
  $('.gallery .link1 span').css('top', 500).css('opacity', 0);
  $('.gallery .link1 span').delay(1500).animate({ top: 450, opacity: 1 }, 'slow');
  $('.gallery .link1 strong').css('top', 450).css('opacity', 0);
  $('.gallery .link1 strong').delay(1000).animate({ top: 340, opacity: 1 }, 'slow');
  ...
  function moveg() {
    ...
    //      $('.btn'+i).css('width','40'); // 버튼 원래의 너비
    //   }

    $('.mbutton').css('background', '#fff'); //버튼불다꺼!!
    $('.mbutton').css('width', '40'); // 버튼 원래의 너비
    $('.btn' + cnt).css('background', '#da291c');//자신만 불켜
    $('.btn' + cnt).css('width', '100');

    $('.gallery li span').css('top', 500).css('opacity', 0);
    $('.gallery .link' + cnt).find('span').delay(1500).animate({ top: 450, opacity: 1 }, 'slow');
    $('.gallery li strong').css('top', 450).css('opacity', 0);
    $('.gallery .link' + cnt).find('strong').delay(1000).animate({ top: 340, opacity: 1 }, 'slow');
