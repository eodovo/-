...
$(document).ready(function () {
    var article = $('.section1 .article');  //모든 질문 답변 리스트
    //article.find('.a').slideUp(100); // 모든 답변 닫아라
    article.find('span').html('<i class="fas fa-chevron-down"></i>');
    $('.section1 .article:first').find('.a').hide();
    $('.section1 .article:first').find('span').html('<i class="fas fa-chevron-down"></i>');

    $('.section1 .article .trigger').click(function (e) {  // 각각의 질문을 클릭하면
